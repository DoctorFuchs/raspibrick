...
def arrow(*args):
    '''
    Draws an arrow from point pt1 = [x1, y1] to pt2 = [x2, y2] 
    @param pt1 the starting point
    @param pt2: the ending point (where is the arrowhead)
    @param size: the length of the arrowhead's triangle (in pixels, default: 10)
...
def doubleArrow(*args):
    '''
    Draws a double arrow from point pt1 = [x1, y1] to pt2 = [x2, y2] 
    (arrowheads on both ends).
    @param pt1 one end of the arrow
    @param pt2: the other end of the arrow
    @param size: the length of the arrowhead's triangle (in pixels, default: 10)
...
#        sys.exit(0)
    ...
    def arrow(self, *args):
        ...
        if len(args) == 5:
            self._arrow([args[0], args[1]], [args[2], args[3]], args[4])
        elif len(args) == 4:
            self._arrow([args[0], args[1]], [args[2], args[3]])
        elif len(args) == 3:
            self._arrow(args[0], args[1], args[2])
        elif len(args) == 2:
            self._arrow(args[0], args[1])

    def _arrow(self, pt0, pt1, size = 10):
        s = self.toUserWidth(size)
        phi = math.atan2(pt1[1] - pt0[1], pt1[0] - pt0[0])
        h = math.sqrt(3) / 2 * s
        tri = [[-h, s/2], [-h, -s/2], [0, 0]]
        tri = self._rotTri(tri, phi)
        tri = self._transTri(tri, pt1)
        self.fillTriangle(tri[0], tri[1], tri[2])
        d = math.sqrt((pt0[0] - pt1[0]) * (pt0[0] - pt1[0]) + (pt0[1] - pt1[1]) * (pt0[1] - pt1[1]))
        p = self.getDividingPoint(pt0[0], pt0[1],  pt1[0], pt1[1], (d - h) / d)
        self.line(pt0[0], pt0[1], p[0], p[1])

    def doubleArrow(self, *args):
        '''
        Draws a double arrow from point pt1 = [x1, y1] to pt2 = [x2, y2] 
        (arrowheads on both ends).
        @param pt1 one end of the arrow
        ...
        if len(args) == 5:
            self._doubleArrow([args[0], args[1]], [args[2], args[3]], args[4])
        elif len(args) == 4:
            self._doubleArrow([args[0], args[1]], [args[2], args[3]])
        elif len(args) == 3:
            self._doubleArrow(args[0], args[1], args[2])
        elif len(args) == 2:
            self._doubleArrow(args[0], args[1])

    def _doubleArrow(self, pt0, pt1, size = 10):
        s = self.toUserWidth(size)
        phi = math.atan2(pt1[1] - pt0[1], pt1[0] - pt0[0])
        h = math.sqrt(3) / 2 * s
        tri = [[-h, s/2], [-h, -s/2], [0, 0]]
        tri1 = self._rotTri(tri, phi)
        tri1 = self._transTri(tri1, pt1)
        self.fillTriangle(tri1[0], tri1[1], tri1[2])
        tri2 = self._rotTri(tri, math.pi + phi)
        tri2 = self._transTri(tri2, pt0)
        self.fillTriangle(tri2[0], tri2[1], tri2[2])
        d = math.sqrt((pt0[0] - pt1[0]) * (pt0[0] - pt1[0]) + (pt0[1] - pt1[1]) * (pt0[1] - pt1[1]))
        p1 = self.getDividingPoint(pt0[0], pt0[1],  pt1[0], pt1[1], (d - h) / d)
        p0 = self.getDividingPoint(pt0[0], pt0[1],  pt1[0], pt1[1], h / d)
        self.line(p0[0], p0[1], p1[0], p1[1])

    def _rotTri(self, tri, phi):
