...
class MyRobot():
    ...
    def __init__(self, ipAddress = ""):
        ...
        if ipAddress == "":
            self.ipAddress = self.panel.askIPAddress()
            if self.ipAddress == "":
        ...
        self.panel.addButtonKeyListener(onKeyEvent)
        msg = "Connecting to " + self.ipAddress + ":" + str(PORT) + "..."
        print msg,
        self.panel.setText(msg)
        if self.connect():
            print "Connection established."
            self.panel.showConnect(self.ipAddress)
            Tools.delay(4000) # wait until "Conn" timed message has finished
        else:
            self.panel.showFail(self.ipAddress, PORT)
